...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchArticles() {
      try {
        setLoading(true);
        // Appel au vrai backend Java
        const response = await ArticleService.getAll(0, 12);
        setArticles(response.content || []);
        setError(false);
      } catch (e) {
        console.error("Erreur de récupération des articles:", e);
        setError(true);
      } finally {
        setLoading(false);
      }
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-slate-950">
          {activeView === 'feed' ? (
            <div className="max-w-7xl mx-auto p-8">
              <div className="mb-8">
                <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Dernières Actualités</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Vos sources analysées et vectorisées en temps réel.</p>
              </div>
